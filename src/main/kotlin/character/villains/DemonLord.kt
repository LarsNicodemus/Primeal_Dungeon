...
class DemonLord(name: String = randomVillainName()) : Villain(name) {
    ...
    fun darkSword(opponent: Hero) {
        ...
        if (opponent.shield > 0) {
            printlnWithDelay(
                "Demonlord $name tried to use Dark Sword on ${opponent.name}, attack was blocked! No damage taken.",
                15
            )
            ...
            printlnWithDelay(
                "Demonlord $name used Dark Sword on ${opponent.name} and inflicted ${
                    roundDouble(
                        attackPower
                    )
    ...
    fun hellFlame(opponent: Hero) {
        ...
        if (opponent.shield > 0) {
            printlnWithDelay(
                "Demonlord $name tried to use Hell Flame on ${opponent.name}, attack was blocked! No damage taken.",
                15
            )
            opponent.blockCounter++
        } else {
            ...
            printlnWithDelay(
                "Demonlord $name used Hell Flame on ${opponent.name} and inflicted ${
                    roundDouble(
                        attackPower
                    )
                } damage. ${hpLeft(opponent.hp)} Health Points left.", 15
    ...
    fun gravityBomb(opponent: Hero) {
        ...
        if (opponent.shield > 0) {
            printlnWithDelay(
                "Demonlord $name tried to use Gravity Bomb on ${opponent.name}, attack was blocked! No damage taken.",
                15
            )
            opponent.blockCounter++
        } else {
            super.magicAttack(opponent, attackPower)
            printlnWithDelay(
                "Demonlord $name used Gravity Bomb on ${opponent.name} and inflicted ${
                    roundDouble(
                        attackPower
                    )
                } damage. ${hpLeft(opponent.hp)} Health Points left.", 15
    ...
    fun rulersGrip(opponent: Hero) {
        ...
        if (opponent.shield > 0) {
            printlnWithDelay(
                "Demonlord $name tried to use Ruler's Grip on ${opponent.name}, attack was blocked! No damage taken.",
                15
            )
            opponent.blockCounter++
        } else {
            super.swordAttack(opponent, attackPower)
            printlnWithDelay(
                "Demonlord $name used Ruler's Grip on ${opponent.name} and inflicted ${
                    roundDouble(
                        attackPower
                    )
                } damage. ${hpLeft(opponent.hp)} Health Points left.", 15
