...
class FirstHeavenlyKing(name: String = randomDemonVillainNameFirst()) : Villain(name) {
    ...
    fun bite(opponent: Hero) {
        companionAttackArt(2, 1)
        attackPower = actualAttackPower(attackPower, attackFactor)
        if (opponent.shield > 0) {
            printlnWithDelay(
                "First Heavenly King $name tried to use Bite on ${opponent.name}, attack was blocked! No damage taken.",
                15
            )
            ...
            printlnWithDelay(
                "First Heavenly King $name used Bite on ${opponent.name} and inflicted ${
                    roundDouble(
                        attackPower
                    )
    ...
    fun bloodLetting(opponent: Hero, companion: Villain) {
        companionAttackArt(2, 2)
        attackPower = actualAttackPower(attackPower, attackFactor)
        if (opponent.shield > 0) {
            printlnWithDelay(
                "First Heavenly King $name tried to use Blood Letting on ${opponent.name}, attack was blocked! No damage taken.",
                15
            )
            opponent.blockCounter++
        } else {
            ...
            printlnWithDelay(
                "First Heavenly King $name used Bloodletting on ${opponent.name}, healed ${companion.name} to a total of ${
                    roundDouble(
                        companion.hp
                    )
                } Health Points with the inflicted damage of ${roundDouble(attackPower)}. ${hpLeft(opponent.hp)} Health Points left.",
                15
            )
    ...
    fun darkHeal(companions: List<Villain>) {
        ...
        printlnWithDelay(
            "First Heavenly King $name used Dark Heal on $healedCompanions and healed each with ${
                roundDouble(
                    healPower
                )
                ...
                companions.joinToString(
                    ...
                    "",
                    "."
                ) { it.name + " " + roundDouble(it.hp) + " Health Points" }
            }", 15
        )
    }

    fun bloodRain(opponents: List<Hero>) {
        companionAttackArt(2, 4)
        ...
        if (unblockedOpponents.isNotEmpty()) {
            ...
            printlnWithDelay(
                "First Heavenly King $name used Blood Rain on ${unblockedOpponents.joinToString { it.name }} and inflicted ${
                    roundDouble(
                        totalDamage
                    )
            ...
            "First Heavenly King $name tried to use Blood Rain on ${blockedOpponents.joinToString { it.name }}, all attacks were blocked! No damage taken.",
            15
        )
        blockedOpponents.forEach { savior -> savior.blockCounter++ }
    }
